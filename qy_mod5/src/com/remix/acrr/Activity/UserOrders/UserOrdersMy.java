...
public class UserOrdersMy extends Activity {

	@ViewInject(R.id.UserOrdersAllBack)
	ImageView							UserOrdersAllBackBtn;
	
	@ViewInject(R.id.myOrdersListView)
	PullToRefreshListView				myOrdersListView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.user_orders_my);
		x.view().inject(this);
		UserOrdersAllBackBtn.setOnClickListener(new MyAndUtils.MyFinishClickListener(this));
		init();
	}
	public void init(){
		myOrdersListView.setMode(Mode.PULL_FROM_START);
		myOrdersListView.setScrollingWhileRefreshingEnabled(true);
		myOrdersListView.setOnRefreshListener(new OnRefreshListener<ListView>() {
			@Override
	...
	private void initData(){
		String url = CONST.host + CONST.getMyOrders + "?" + CONST.MYTELID_STRING + "=" + CONST.userInfo.getTel();
		Log.e("DEBUG2", url);
		...
		x.http().get(requestParams, new CommonCallback<String>() {
			...
			public void onSuccess(String arg0) {
				...
				if(list.size() == 0) Toast.makeText(UserOrdersMy.this, "没有相关数据", 0).show();
				myDataList = list;
				shopsAdapter = new ShopsAdapter(myDataList, UserOrdersMy.this, is_loadPIC, true);//已经处理中的单子
				myOrdersListView.setAdapter(shopsAdapter);
				shopsAdapter.notifyDataSetChanged();
				myOrdersListView.onRefreshComplete();
			}
		});
	}
	@Override
